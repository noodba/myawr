...
print  "---------------------------------\n";	
		...
		$sth = $dbh->prepare("SELECT snap_id,snap_time from myawr_snapshot WHERE host_id=$tid and snap_id in ($sql)");
		$sth->execute();
...
<tr><td>Thread_cache_hits %:</td><td align=\"right\">$Thread_cache_hits</td></tr>
    ...
    print MYAWR_REPORT "<p /><h3>Top 10 read file Events</h3><p />   <table border=\"1\" width=\"600\" > <tr><th>event_name</th><th>read bytes</th><th>read count</th></tr>";
		...
		$sth = $dbh->prepare("select  b.EVENT_NAME ,b.SUM_NUMBER_OF_BYTES_READ-a.SUM_NUMBER_OF_BYTES_READ file_read, b.COUNT_READ-a.COUNT_READ read_count from (select * from myawr_snapshot_file_summary_by_event_name where host_id=$tid and snap_id=$start_snap_id) a, (select * from myawr_snapshot_file_summary_by_event_name where  host_id=$tid and snap_id=$end_snap_id) b WHERE a.EVENT_NAME=b.EVENT_NAME order by b.SUM_NUMBER_OF_BYTES_READ-a.SUM_NUMBER_OF_BYTES_READ desc limit 10");
		$sth->execute();
		while( my @result = $sth->fetchrow_array )	{
			  print MYAWR_REPORT "<tr>   <td>$result[0]</td><td align=\"right\">$result[1]</td><td align=\"right\">$result[2]</td></tr>";  
	          print MYAWR_REPORT "\n";
    ...
    print MYAWR_REPORT "</table><p /><hr/><p />";			  	
		...
		$sth = $dbh->prepare("SELECT b.snap_time,a.load1,a.load5,a.load15 from myawr_load_info a,myawr_snapshot b WHERE a.snap_id=b.snap_id and a.host_id=$tid and  b.host_id=a.host_id and a.snap_id in ($sql)");
		$sth->execute();
    ...
    print MYAWR_REPORT "</table><p />";		
		...
		$sth = $dbh->prepare("SELECT b.snap_time,a.cpu_user,a.cpu_system,a.cpu_idle,a.cpu_iowait from myawr_cpu_info a,myawr_snapshot b WHERE a.snap_id=b.snap_id and a.host_id=$tid and  b.host_id=a.host_id and a.snap_id in ($sql)");
		$sth->execute();
    ...
    print MYAWR_REPORT "</table><p />";	
 	print MYAWR_REPORT "\n";   
		...
		$sth = $dbh->prepare("SELECT b.snap_time,a.rd_ios_s,a.wr_ios_s,a.rkbs,a.wkbs,a.queue,a.svc_t,a.busy from myawr_io_info a,myawr_snapshot b WHERE a.snap_id=b.snap_id and a.host_id=$tid and  b.host_id=a.host_id and a.snap_id in ($sql)");
		$sth->execute();
    ...
    print MYAWR_REPORT "</table><p />";	
 		...
 		$sth = $dbh->prepare("SELECT b.snap_time,a.swap_in,a.swap_out,a.net_recv,a.net_send,a.file_system,a.total_mb,a.used_mb,a.used_pct,a.mount_point from myawr_swap_net_disk_info a,myawr_snapshot b WHERE a.snap_id=b.snap_id and a.host_id=$tid and  b.host_id=a.host_id and a.snap_id in ($sql)");
		$sth->execute();
